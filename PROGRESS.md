...
| 1.2 | 编写数据下载脚本（MET API），获取 1-5 万张文物图片 + 元数据 | ✅ | 2026-08-10 | 下载 2051 件（关键词搜索方案 B + 断点续传，2026-08-10 补数据完成）；数据量较初版扩充约 7 倍 |
| 1.3 | 下载 DINOv2 / CLIP 模型，为图片提取特征向量，存为 Parquet | ✅ | 2026-08-10 | DINOv2 提取 2051 张 → (2051,768) L2 归一化 → parquet/npy/id_map |
| 1.4 | 构建 FAISS 索引（HNSW），关联向量与图像路径 | ✅ | 2026-08-10 | HNSW(IP/余弦) 2051 向量，自测 5/5，真实检索 Top-5 相似度 0.60-1.0 |
| 1.5 | 从 Wikidata 抽取文物 KG 三元组，导入 Neo4j | ✅ | 2026-08-10 | 本地 2051 件 + Wikidata 补全时期/文化(114/245匹配)；节点 2051+145+107，关系 1281 |
...
- [x] 2026-08-10：Gradio 界面优化 + 证据链网络图
  - 界面：标题描述（RAG 流程）、上传引导、操作提示
  - 证据链图：src/viz/evidence.py（networkx + matplotlib）查询→相似文物→时期/文化
  - 验证：浏览器显示证据链网络图正常
- [x] 2026-08-10：Docker 部署完善（4.3）
